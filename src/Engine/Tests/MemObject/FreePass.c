...
int main()
{
   ...
   vdseFree( pObj, buff[1], VDSE_PAGE_SIZE, &context );
   vdseFree( pObj, buff[2], VDSE_PAGE_SIZE, &context );
   vdseFree( pObj, buff[0], VDSE_PAGE_SIZE, &context );
   if ( pObj->totalPages != 4 ) 
      ERROR_EXIT( expectedToPass, NULL, ; );
   
   buff[0] = vdseMalloc( pObj, 3*VDSE_PAGE_SIZE, &context );
   if ( buff[0] == NULL )
      ERROR_EXIT( expectedToPass, &context.errorHandler, ; );
   if ( pObj->totalPages != 4 )
      ERROR_EXIT( expectedToPass, NULL, ; );
   ...
   vdseFree( pObj, buff[3], VDSE_PAGE_SIZE, &context );
   if ( pObj->totalPages != 4 )
      ERROR_EXIT( expectedToPass, &context.errorHandler, ; );

                                             ...
                                             jobject  entry )
{
   ...
   if ( errcode == PSO_OK ) {
      ...
      (*env)->SetObjectField( env, entry, g_idEntryName,   jname );
      (*env)->SetObjectField( env, entry, g_idEntryType,   g_weakObjType[cEntry.type-1] );
      (*env)->SetIntField(    env, entry, g_idEntryStatus, cEntry.status );
   }

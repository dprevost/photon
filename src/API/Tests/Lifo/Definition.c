...
int main( int argc, char * argv[] )
{
   ...
   errcode = vdsLifoPush( objHandle, data1, lenData );
   if ( errcode != VDS_OK ) {
      fprintf( stderr, "err: %d\n", errcode );
      ERROR_EXIT( expectedToPass, NULL, ; );
   }
   ...
   errcode = vdsLifoDefinition( NULL, &pDef );
   if ( errcode != VDS_NULL_HANDLE ) {
      fprintf( stderr, "err: %d\n", errcode );
      ERROR_EXIT( expectedToPass, NULL, ; );
   }

   errcode = vdsLifoDefinition( objHandle, NULL );
   ...
   errcode = vdsLifoDefinition( objHandle, &pDef );
   if ( errcode != VDS_OK ) {
      fprintf( stderr, "err: %d\n", errcode );
      ERROR_EXIT( expectedToPass, NULL, ; );
   }

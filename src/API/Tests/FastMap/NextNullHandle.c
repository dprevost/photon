...
int main( int argc, char * argv[] )
{
   ...
   errcode = psoCreateFolder( sessionHandle,
                              "/api_fast_map_next_null_handle",
                              strlen("/api_fast_map_next_null_handle") );
   if ( errcode != PSO_OK ) {
      fprintf( stderr, "err: %d\n", errcode );
      ERROR_EXIT( expectedToPass, NULL, ; );
   }
   ...
   errcode = psoKeyDefCreate( sessionHandle,
                              "api_fastmap_next_null_handle",
                              strlen("api_fastmap_next_null_handle"),
   ...
   errcode = psoCreateKeyedObject( sessionHandle,
                                   "/api_fast_map_next_null_handle/test",
                                   strlen("/api_fast_map_next_null_handle/test"),
                                   &mapDef,
                                   dataDefHandle,
                                   keyDefHandle );
   if ( errcode != PSO_OK ) {
   ...
   errcode = psoaFastMapNext( NULL, &keyBuffer, &keyLength, 
                              &buffer, &bufferLength );

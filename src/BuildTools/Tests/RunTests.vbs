...
if consoleMode then 
   ...
   if verbose then 
      WScript.Echo objWshScriptExec.StdErr.ReadAll
   end if
...
if consoleMode then 
   WScript.Echo "Running " & exe_name
   Set objWshScriptExec = objShell.Exec("%comspec% /c " & Chr(34) & exe_name & Chr(34) & " --test")
   status = objWshScriptExec.Status
   Do While objWshScriptExec.Status = 0
      WScript.Sleep 100
   Loop
   ...
   rc = objShell.Run("%comspec% /c " & Chr(34) & exe_name & Chr(34) & " --test", 2, true)
end if
if rc <> 0 then
   failed_tests(numFailed) = program
   numFailed = numFailed + 1

...
void test_null_handle( void ** state )
{
   ...
   expect_assert_failure( psocIsLongOptPresent( NULL, "address" ) );
#endif
   return;
}
...
{
#if defined(PSO_UNIT_TESTS)
   expect_assert_failure( psocIsLongOptPresent( handle, NULL ) );
#endif
   return;
}
...
void test_pass( void ** state )
{
   ...
   /* Option is present */
   gotIt = psocIsLongOptPresent( handle, "address" );
   assert_true( gotIt );
   
   /* Option is absent */
   gotIt = psocIsLongOptPresent( handle, "three" );
   assert_false( gotIt );
   
   /* Unknown option */
   gotIt = psocIsLongOptPresent( handle, "zzaddress" );
   assert_false( gotIt );

...
void test_invalid_sig( void ** state )
{
   ...
   expect_assert_failure( psocSetError( &errorHandler, PSOC_ERRNO_HANDLE, ENOENT ) );
...
void test_invalid_handle( void ** state )
{
#if defined(PSO_UNIT_TESTS)
   expect_assert_failure( psocSetError( &errorHandler, 123, ENOENT ) );
#endif
   return;
}
...
{
#if defined(PSO_UNIT_TESTS)
   expect_assert_failure( psocSetError( NULL, PSOC_ERRNO_HANDLE, ENOENT ) );
#endif
   return;
}
...
int main()
{
   ...
   const UnitTest tests[] = {
      ...
      unit_test_setup_teardown( test_invalid_handle, setup_test, teardown_test ),

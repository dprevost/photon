...
ok_programs(3) = "LockTests"
ok_programs(4) = "ReleasePass"
ok_programs(5) = "TryAcqPass"
...
exe_name = prog_path & "\LockConcurrency.exe 25 . try"
if consoleMode then 
   WScript.Echo "Running " & exe_name
   Set objWshScriptExec = objShell.Exec("%comspec% /c " & Chr(34) & exe_name & Chr(34))
   status = objWshScriptExec.Status
...
exe_name = prog_path & "\LockConcurrency.exe 25 ."
if consoleMode then 
   WScript.Echo "Running " & exe_name
   Set objWshScriptExec = objShell.Exec("%comspec% /c " & Chr(34) & exe_name & Chr(34))
   status = objWshScriptExec.Status

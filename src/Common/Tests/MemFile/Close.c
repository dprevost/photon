...
void test_invalid_sig( void ** state )
{
   ...
   mem.initialized = 0;
   expect_assert_failure( psocCloseMemFile( &mem, &errorHandler ) );
...
void test_null_error( void ** state )
{
   ...
   expect_assert_failure( psocCloseMemFile( &mem, NULL ) );
#endif
   return;
}
...
void test_null_mem( void ** state )
{
   ...
   expect_assert_failure( psocCloseMemFile( NULL, &errorHandler ) );
#endif
   return;
}

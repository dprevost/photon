...
void setup_test()
{
   ...
   psocInitMemoryFile( &mem, 29, "MemFile.mem" );
}
...
void test_invalid_sig( void ** state )
{
   ...
   mem.initialized = 0;
   expect_assert_failure( psocCopyBackstore( &mem, 0755, &errorHandler ) );
...
void test_null_error( void ** state )
{
   ...
   expect_assert_failure( psocCopyBackstore( &mem, 0755, NULL ) );
#endif
   return;
}
...
void test_null_mem( void ** state )
{
   ...
   expect_assert_failure( psocCopyBackstore( NULL, 0755, &errorHandler ) );
#endif
   return;
}
...
void test_perm_0000( void ** state )
{
   ...
   expect_assert_failure( psocCopyBackstore( &mem, 0000, &errorHandler ) );
#endif
   return;
}
...
void test_perm_0200( void ** state )
{
   ...
   expect_assert_failure( psocCopyBackstore( &mem, 0200, &errorHandler ) );
#endif
   return;
}
...
void test_perm_0400( void ** state )
{
   ...
   expect_assert_failure( psocCopyBackstore( &mem, 0400, &errorHandler ) );
#endif
   return;
}
...
void test_perm_0500( void ** state )
{
   ...
   expect_assert_failure( psocCopyBackstore( &mem, 0500, &errorHandler ) );
#endif
   return;
}

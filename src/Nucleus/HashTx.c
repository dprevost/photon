                               ...
                               psonSessionContext * pContext )
{
   ...
   psonHashTxItem * pOldItem, * pNewItem, * previousItem, * tempItem;
   ...
   PSO_INV_CONDITION( pOldHash->initialized == PSON_HASH_TX_SIGNATURE );
   PSO_INV_CONDITION( pNewHash->initialized == PSON_HASH_TX_SIGNATURE );
   ...
   if ( pOldHash->lengthIndex == pNewHash->lengthIndex ) {
      ...
      for ( i = 0; i < g_psonArrayLengths[pOldHash->lengthIndex]; ++i ) {
         ...
         previousItem = NULL;
         while ( currentOffset != PSON_NULL_OFFSET ) {
            GET_PTR( pOldItem, currentOffset, psonHashTxItem );

            itemLength = calculateItemLength( pOldItem->keyLength,
                                              pOldItem->dataLength );
            pNewItem = (psonHashTxItem*) psonMalloc( pMemObject, 
                                                   itemLength, 
                                                   pContext );
      ...
      for ( i = 0; i < g_psonArrayLengths[pOldHash->lengthIndex]; ++i ) {
         ...
         while ( currentOffset != PSON_NULL_OFFSET ) {
            ...
            if ( pNewArray[newBucket] == PSON_NULL_OFFSET ) {
               ...
               GET_PTR( tempItem, tempOffset, psonHashTxItem );
                           ...
                           psonSessionContext  * pContext  )
{
   ...
   ptrdiff_t * pArray;
   bool keyFound = false;
   psonHashTxItem * previousItem = NULL, * pItem = NULL;
   ptrdiff_t nextOffset;
   psonMemObject * pMemObject;
...
enum psoErrors 
psonHashTxUpdate( psonHashTx          * pHash,
                  const unsigned char * pKey,
                  size_t                keyLength,
                  const void          * pData,
                  size_t                dataLength,
                  psonSessionContext  * pContext )
{
   ...
   psonHashTxItem * pOldItem, * previousItem = NULL, * pNewItem = NULL;
   psonMemObject * pMemObject;

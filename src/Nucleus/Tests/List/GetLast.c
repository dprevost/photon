...
void test_invalid_sig( void ** state )
{
   ...
   list.initialized = 0;
   expect_assert_failure( psonLinkedListGetLast( &list, &pNode ) );
...
{
#if defined(PSO_UNIT_TESTS)
   expect_assert_failure( psonLinkedListGetLast( &list, NULL ) );
#endif
   return;
}
...
{
#if defined(PSO_UNIT_TESTS)
   expect_assert_failure( psonLinkedListGetLast( NULL, &pNode ) );
#endif
   return;
}

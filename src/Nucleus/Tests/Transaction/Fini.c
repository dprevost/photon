...
void test_invalid_sig( void ** state )
{
   ...
   value = pTx->signature;
   pTx->signature = 0;
   expect_assert_failure( psonTxFini( pTx, &context ) );
   pTx->signature = value;
#endif
   return;
}
...
{
#if defined(PSO_UNIT_TESTS)
   expect_assert_failure( psonTxFini( pTx, NULL ) );
#endif
   return;
}
...
{
#if defined(PSO_UNIT_TESTS)
   expect_assert_failure( psonTxFini( NULL, &context ) );
#endif
   return;
}

...
void test_null_context( void ** state )
{
   ...
   expect_assert_failure( psonHashInit( pHash, g_memObjOffset, 100, NULL ) );
#endif
   return;
}
...
{
#if defined(PSO_UNIT_TESTS)
   expect_assert_failure( psonHashInit( NULL, g_memObjOffset, 100, &context ) );
#endif
   return;
}
...
{
#if defined(PSO_UNIT_TESTS)
   expect_assert_failure( psonHashInit( pHash, PSON_NULL_OFFSET, 100, &context ) );
#endif
   return;
}
